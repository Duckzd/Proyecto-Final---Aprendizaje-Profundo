...
$0{,}0077$ en el espacio normalizado, con errores de $\mathrm{MAE}=1{,}48$ y
$\mathrm{RMSE}=2{,}27$ en unidades físicas, y genera animaciones autoregresivas
...
A diferencia de esos trabajos, nuestro escenario es 2D y controlado: el generador
procedural provee datos ilimitados y sin ruido, lo que aísla el efecto de la
arquitectura del de la calidad de los datos.
...
El render cierra el ciclo de validación: sólo viendo el movimiento se distingue un
error irrelevante de otro que arruina la pose. El proceso es idéntico para el
generador procedural y para la red (Fig.~\ref{fig:pipeline}): se recorre el árbol
con las ecuaciones (1) y (2) para obtener el ángulo y la posición de los 17 nodos;
a cada hueso le corresponde una imagen PNG (generada también de forma procedural)
que se rota $\phi_j$ grados alrededor de su pivote y se ancla sobre la articulación
padre, de modo que las piezas nunca se separan; finalmente se componen de atrás
hacia delante. Lo esencial es que el motor de render desconoce si el estado
procede de una fórmula o de una red, por lo que sustituir el generador por el
modelo entrenado no obligó a modificarlo.
...
\end{equation}
que vale cero en los extremos y alcanza su máximo en el golpe. Los ángulos de la
pierna derecha interpolan entre reposo e impacto con ese factor:
\begin{align}
\text{right\_hip}(t)  &= \mathrm{lerp}(-10^\circ,\ r,\ e(t)),\\
\text{right\_knee}(t) &= \mathrm{lerp}(70^\circ,\ 8^\circ,\ e(t)),
\end{align}
de modo que la cadera eleva la pierna al frente mientras la rodilla se extiende, y
ambas regresan a la posición inicial.

Un aspecto esencial es que los parámetros se \textbf{sortean al azar en cada
ejecución}: en la patada varían el instante del golpe $t_g\sim U(0{,}45,\,0{,}60)$,
la velocidad del ataque (que fija el ancho $\sigma$) y el alcance
$r\sim U(85^\circ,105^\circ)$; en la marcha, la longitud del paso, la amplitud de
los brazos y la inclinación del torso; en el salto, su altura. Así nunca se
generan dos secuencias idénticas, lo que impide memorizar trayectorias y actúa
como una forma intensiva de aumento de datos.
...
temporal} de esa única articulación a lo largo de toda la patada: una curva
ascendente y descendente que reproduce la mecánica del golpe.
...
unidades durante 16 épocas, alcanzando una pérdida final de $0{,}0163$.
...
validación desciende de $0{,}3126$ a $0{,}0077$ a lo largo de 15 épocas de forma
...
expresado en unidades físicas es de $\mathrm{MAE}=1{,}48$ y
$\mathrm{RMSE}=2{,}27$. Dado que los ángulos recorren rangos de hasta
$150^\circ$, un error medio próximo a $1{,}5^\circ$ resulta visualmente
imperceptible.
...
un MAE de $1{,}64$ frente a $1{,}66$, pero la GRU entrena cerca de un
17\,\% más rápido por disponer de menos parámetros por celda. La RNN simple es la
...
\midrule
RNN  & 1{,}97 & 0{,}0037 & 0{,}0143 & 1{,}938 & 3{,}630 \\
LSTM & 6{,}47 & 0{,}0075 & \textbf{0{,}0104} & \textbf{1{,}641} & \textbf{2{,}897} \\
GRU  & 5{,}39 & 0{,}0072 & 0{,}0109 & 1{,}663 & 3{,}065 \\
\bottomrule
\end{tabular}
\end{table}
...
reproduciendo la mecánica del golpe. La
...
%=====================================================================
Cada invocación devuelve un vector de 17 números normalizados: la pose siguiente,
no una imagen ni una clase. Se interpreta deshaciendo la normalización
($x=\tilde{x}\sigma+\mu$) y leyendo cada componente según la
Tabla~\ref{tab:estado}. La animación completa se obtiene de forma autoregresiva:
se parte de una ventana semilla, se predice la pose siguiente, se añade a la
ventana y se descarta la más antigua; cada pose se satura a los límites
articulares, se convierte en coordenadas por cinemática directa y se dibuja. El
visualizador se lanza con \texttt{python main.py} y el cuaderno
\texttt{development.ipynb} reproduce dataset, entrenamiento, comparación y
generación; los artefactos \texttt{best\_model.pt} y \texttt{normalizer.npz}
permiten inferir sin reentrenar. La implementación emplea PyTorch.
...
Las celdas con compuertas superan a la RNN simple y resultan equivalentes entre
sí, con ventaja de coste para la GRU; las diez acciones son reconocibles y el
condicionamiento por acción permite encadenar coreografías a demanda.
